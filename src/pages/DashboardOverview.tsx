...
import { getPharmacyBranches } from '../services/branchService';
...
const DashboardOverview: React.FC = () => {
  ...
  // Fetch branches data (must be inside component)
  const { data: branches = [] } = useQuery({
    queryKey: ['pharmacy-branches'],
  ...
  const medicineActivities = medicines.map(med => ({
    type: 'medicine',
    id: med.id,
    description: `Medicine "${med.name}" added/updated`,
    extra: `${med.quantity} units, $${med.price}`,
  ...
  const orderActivities = orders.map(order => ({
    type: 'order',
    id: order.id,
    description: `Order #${order.id} ${order.status.toLowerCase() === 'completed' ? 'completed' : 'placed'}`,
    extra: `Customer: ${order.user_email}`,
    created_at: order.created_at,
  }));

  const branchActivities = branches.map(branch => ({
    type: 'branch',
    id: branch.id,
    description: `Branch "${branch.branch_name}" created/updated`,
    extra: branch.location,
    created_at: branch.created_at,
  }));

  const allActivities = [...medicineActivities, ...orderActivities, ...branchActivities]
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 text-sm md:text-base">
              {formatters.date(dayjs())} • Pharmacy Dashboard
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4">
          {allActivities.length === 0 ? (
            ...
            allActivities.map((activity) => (
              <div key={activity.type + '-' + activity.id + '-' + activity.created_at} className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl">
                <div className={`p-2 rounded-lg ${activity.type === 'order' ? 'bg-green-100' : activity.type === 'medicine' ? 'bg-blue-100' : 'bg-orange-100'}`}>
                  {activity.type === 'order' ? <ShoppingCart className="w-4 h-4 text-green-600" /> : activity.type === 'medicine' ? <Package className="w-4 h-4 text-blue-600" /> : <Clock className="w-4 h-4 text-orange-600" />}
                </div>
                <div className="flex-1">
                  <p className="font-medium text-gray-900">{activity.description}</p>
                  <p className="text-sm text-gray-600">{activity.extra}</p>
                </div>
                <span className="text-sm text-gray-500">{dayjs(activity.created_at).fromNow()}</span>
              </div>
            ))
          )}
        </div>
