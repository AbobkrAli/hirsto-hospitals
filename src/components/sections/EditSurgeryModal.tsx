...
import { useUpdateSurgery, type UpdateSurgeryRequest, type SurgeryData } from '../../services/surgeriesService';
...
const EditSurgeryModal: React.FC<EditSurgeryModalProps> = ({ isOpen, onClose, onSuccess, surgery }) => {
  const [form, setForm] = useState<UpdateSurgeryRequest>({
    name: '',
    description: '',
  ...
  useEffect(() => {
    if (isOpen && surgery) {
      setForm({
        ...
        specialization_required: surgery.specialization_required || '',
        estimated_duration_hours: surgery.estimated_duration_hours || 1,
        cost: surgery.cost || 1,
        preparation_instructions: surgery.preparation_instructions || '',
        post_surgery_care: surgery.post_surgery_care || '',
        risk_level: surgery.risk_level || 'medium',
        is_emergency_surgery: !!surgery.is_emergency_surgery,
      });
      setErrors({});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!surgery) return;
    const fields: Array<keyof UpdateSurgeryRequest> = [
    ...
    try {
      await updateSurgery.mutateAsync({ surgeryId: surgery.id, data: form });
      onSuccess();
    } finally {
      setIsSubmitting(false);
    }
              ...
              <div className="backdrop-blur-xl rounded-2xl !max-h-[90vh] overflow-y-auto shadow-2xl border bg-white/90 border-[#90E0EF]/30 p-6 ">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-xl font-bold font-headlines text-[#1E3E72]">Edit Surgery</h2>
                      <p className="text-sm text-[#0077B6] font-paragraphs">Update surgery</p>
                    </div>
                  </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    <Button type="submit" disabled={!canSubmit} loading={isSubmitting || updateSurgery.isPending} className="flex-1">Save Changes</Button>
                  </div>
                </form>
              </div>
            </div>
