...
import { useCreateMedicalTest, type CreateMedicalTestRequest } from '../../services/medicalTestsService';
...
const AddMedicalTestModal: React.FC<AddMedicalTestModalProps> = ({ isOpen, onClose, onSuccess }) => {
  const [form, setForm] = useState<CreateMedicalTestRequest>({
    name: '',
    description: '',
    test_type: '',
    department: '',
  ...
  const createMutation = useCreateMedicalTest();

  useEffect(() => {
    if (isOpen) {
      setForm({
        name: '', description: '', test_type: '', department: '', cost: 1, duration_minutes: 1,
        preparation_instructions: '', fasting_required: false, fasting_hours: 0, sample_type: '', normal_range: ''
      });
  ...
  const validateField = (name: string, value: string | number | boolean): string => {
    const required = (v: any) => String(v ?? '').trim() ? '' : 'This field is required';
    switch (name) {
      case 'name':
      case 'description':
      case 'test_type':
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const fields: Array<keyof CreateMedicalTestRequest> = [
      'name', 'description', 'test_type', 'department', 'cost', 'duration_minutes', 'preparation_instructions', 'sample_type', 'normal_range'
    ];
    ...
    if (form.fasting_required) {
      const fhErr = validateField('fasting_hours', form.fasting_hours);
      if (fhErr) newErrors.fasting_hours = fhErr;
    }
    setErrors(newErrors);
    ...
    try {
      ...
      const message = err?.response?.data?.detail || err?.message || 'Failed to add medical test';
  ...
  const canSubmit = !isSubmitting; // Allow click; validate on submit
              ...
              <div className="backdrop-blur-xl rounded-2xl !max-h-[90vh] overflow-y-auto shadow-2xl border bg-white/90 border-[#90E0EF]/30 p-6 ">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-xl font-bold font-headlines text-[#1E3E72]">Add Medical Test</h2>
                      <p className="text-sm text-[#0077B6] font-paragraphs">Create a new medical test</p>
                    </div>
                  </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    <Button type="submit" disabled={!canSubmit || createMutation.isPending} loading={isSubmitting || createMutation.isPending} className="flex-1">Add Test</Button>
                  </div>
                </form>
              </div>
            </div>
