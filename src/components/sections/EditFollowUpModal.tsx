...
const EditFollowUpModal: React.FC<EditFollowUpModalProps> = ({ isOpen, onClose, onSuccess, followUp }) => {
  const [form, setForm] = useState<UpdateFollowUpRequest>({
    doctor_id: 0,
    patient_email: '',
    patient_name: '',
    follow_up_type: '',
  ...
  useEffect(() => {
    if (isOpen && followUp) {
      setForm({
        doctor_id: followUp.doctor_id || 0,
        patient_email: followUp.patient_email || '',
        patient_name: followUp.patient_name || '',
        follow_up_type: followUp.follow_up_type || '',
        related_surgery_id: followUp.related_surgery_id ?? null,
        related_test_id: followUp.related_test_id ?? null,
        initial_consultation_date: followUp.initial_consultation_date || '',
        next_appointment_date: followUp.next_appointment_date || '',
        priority_level: followUp.priority_level || 'routine',
        chief_complaint: followUp.chief_complaint || '',
        current_symptoms: followUp.current_symptoms || '',
        vital_signs: followUp.vital_signs || '',
        current_medications: followUp.current_medications || '',
        treatment_plan: followUp.treatment_plan || '',
        follow_up_frequency: followUp.follow_up_frequency || '',
        total_sessions_planned: followUp.total_sessions_planned || 0,
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!followUp) return;
    const fields: Array<keyof UpdateFollowUpRequest> = [
    ...
    try {
      await updateMutation.mutateAsync({ followUpId: followUp.id, data: form });
      onSuccess();
    } finally {
      setIsSubmitting(false);
    }
                ...
                <div className="relative p-6">
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h2 className="text-xl font-bold font-headlines text-[#1E3E72]">Edit Follow Up</h2>
                        <p className="text-sm text-[#0077B6] font-paragraphs">Update follow up</p>
                      </div>
                    </div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex gap-3 pt-2">
                      ...
                      <Button type="submit" disabled={!canSubmit || updateMutation.isPending} loading={isSubmitting || updateMutation.isPending} className="flex-1">Save Changes</Button>
                    </div>
                  </form>
                </div>
              </div>
